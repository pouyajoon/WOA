...
public abstract class AbstractExample
{
	...
	public ConcreteBoundaryListener m_boundaryListener;
	...
	//public float cachedCamX = 0, cachedCamY = 0, cachedCamScale = 0.1f;
...
//		} else {
//			m_debugDraw.setCamera(500.0f, 10.0f, 0.2f);
...
//		}
	...
	/**
	 * Draw a rectangle providing coordinates
	 * @param bottomdown
	 * @param widthheight
	 */
	public void drawRectangle(Vec2 bottomdown, Vec2 widthheight)
	{
		// top
		m_debugDraw.drawSegment(bottomdown, new Vec2(bottomdown.x + widthheight.x, bottomdown.y), green);
		// bottom
		m_debugDraw.drawSegment(new Vec2(bottomdown.x, bottomdown.y + widthheight.y), new Vec2(bottomdown.x + widthheight.x, bottomdown.y + widthheight.y), green);
		//left
		m_debugDraw.drawSegment(bottomdown, new Vec2(bottomdown.x, bottomdown.y + widthheight.y), green);		
		//right
		m_debugDraw.drawSegment(new Vec2(bottomdown.x + widthheight.x, bottomdown.y), new Vec2(bottomdown.x + widthheight.x, bottomdown.y + widthheight.y), green);
	}
	...
	public void step() 
	{
		...
		mouseWorld.set(m_debugDraw.getScreenToWorld(mouseScreen));
...
//    
    ...
    public void mouseDown(Vec2 p) {
    	...
    	p = m_debugDraw.getScreenToWorld(p);
        ...
        for (int j = 0; j < shapes.length; j++) {
            ...
            {
            	System.out.println("dynamic body clicked : " + shapeBody.getUserData().toString());
            ...
            {
            	if (shapeBody.getUserData() != null)
            	System.out.println("static body clicked : " + shapeBody.getUserData().toString());
            }
        }
        ...
        if (body != null) {
            ...
            md.body2 = body;
            md.target.set(p);
            md.maxForce = 1000000.0f * body.m_mass;
    ...
    public void mouseMove(Vec2 p) {
    	mouseScreen.set(p);
        if (m_mouseJoint != null) {
            m_mouseJoint.setTarget(m_debugDraw.getScreenToWorld(p));
        }
    }
...
//    }
    ...
    public class ConcreteBoundaryListener implements BoundaryListener 
    {
		...
		public void violation(Body body) {
    		try {
    			/*
    			WOAObject woao = (WOAObject)body.getUserData();
        		System.out.println("boundary violation : " + body.getPosition().toString() + ";" + woao.currentBox.getBoundarySide(body.getPosition()));
        		int direction = woao.currentBox.getBoundarySide(body.getPosition());
        		AntBox boxTarget = woao.currentBox.world.antzone.getTargetDirection(woao.currentBox, direction);
        		System.out.println("new adviced box id : " + boxTarget.id + " for direction : " + direction);
        		woao.destroyBody();
        		*/
    			...
    			{
    				WOAObject woao = (WOAObject)body.getUserData();
    				parent.antworld.hasViolateBoundary.put(body.hashCode(), woao);
        			
        			//woao.boundaryViolated();
    			}
//        		test.settings.pause = true;
        		//woao.createBody(boxTarget);
        		
        		
        		//WOAObject woaobject = (WOAObject)body.getUserData();
				//
			} catch (Throwable e) {
    ...
    class ConcreteContactListener implements ContactListener {
    	public void add(org.jbox2d.dynamics.contacts.ContactPoint point) {
    		...
    		WOAObject one = (WOAObject) point.shape1.getBody().getUserData();
    		WOAObject two = (WOAObject) point.shape2.getBody().getUserData();
    		
    		if (one != null && two != null)
    		{
    			//System.out.println("contact between : " + one.toString() + " and " + two.toString());
    			one.collide(two, point);
    			two.collide(one, point);
    		}
    		//+ " with " + point.shape1.getUserData().toString()
    	...
    	public void persist(org.jbox2d.dynamics.contacts.ContactPoint point) {
    		...
    		//System.out.println("collision persist");
    		...
    		{
    			//System.out.println("contact between : " + one.toString() + " and " + two.toString());
    			one.collisionPersists(two);
    			two.collisionPersists(one);
    		}    		
    	}
