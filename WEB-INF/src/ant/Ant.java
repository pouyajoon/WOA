...
public class Ant extends WOAObject{
	...
	public Ant(Colony c)
	{			
		//super(c.world, new Position (c.getPos().get_map(), c.getPos().getX() + 100.0f, c.getPos().getY() + 50.0f));
//		Position p = ;
	...
	{
//		if (myBody.getJointList() == null)
//		{
//			System.out.println("no joint");
//		}
//		System.out.println("destroy body : "  + ", "+ ((WOAObject)myBody.getUserData()).id + ", " + myBody.hashCode());
	...
	public void boundaryViolated()
	{
		...
		//myBody.m_sleepTime = 1;
		//System.out.println("boundary violation : " + myBody.getPosition().toString() + ";" + currentBox.getBoundarySide(myBody.getPosition()));
		...
		//woao.UpdateBodyPosition();
		pos =  currentBox.world.antzone.getTargetPosition(currentBox, boxTarget, direction, myBody.getPosition());
		//System.out.println("new adviced box id : " + boxTarget.id + " for direction : " + direction + ", woaoPos : " + myBody.getPosition() + ", new pos : "+ pos.toString());
		destroyBody();
	...
	public void eatFood(WOAObject other)
	{
		...
		//System.out.println(ant.toString() + " eats " + other.toString());
		...
		//System.out.println("ant_food_place : " + ant_food_place);	
		...
		//System.out.println("ant_food_recolt_capacity : " + ant_food_recolt_capacity);		
	...
	public AABB viewZone()
	{
		...
		//System.out.println("ant currentBox id :" + currentBox.id);
		if (currentBox.world.parent.currentTest.equals(currentBox))
		{
			//currentBox.m_debugDraw.drawPolygon(vsw, 4, new Color3f(255.0f*0.3f, 255.0f*0.9f, 255.0f*0.9f));	
		}
        ...
        for (int j = 0; j < shapes.length; j++) {
            ...
            if (shapeBody.isStatic() == false)
            {
            	if (shapes[j].m_body.getUserData() != null)
            	{
            		...
            		//System.out.println("see : " + woaobject.myBody.getUserData().toString());
            		for (String t : search_types)
            		{
            			if (woaobject.type.equals(t))
            			{
                			//System.out.println("see : " + shapes[j].m_body.getUserData().toString());
	...
	public void collide(WOAObject other, ContactPoint cp)
	{
		...
		{
			//System.out.println(this.toString() + " catch " + other.toString());
			// catch it
			//catchWithMandibule(other);
		...
		{
			currentMission.collisionWithTarget(other, cp);
			
			if (other.equals(getColony()))
			{
				if (currentMission != null)
				{
					//System.out.println("Collision with COLONY");
					currentMission.collisionWithColony();					
				}
				
			}			
			if (other.type.equals("colony"))
			{
				if (currentMission != null)
				{
					//System.out.println("Collision with COLONY");
//					currentMission.collisionWithColony();					
	...
	{		
		...
		if (mandible == null)
		{
			...
			//System.out.println("contact point : " + position.toString());
	...
	public WOAObject searchForFood()
	{
		...
		if (woaobject_list != null)
		{
			if (woaobject_list.size() == 1)
			{
				...
				//System.out.println("my new target is : " + target.toString());
				// go catch it.	
				//state = 1;
				return target;
			}
	...
	public void roam()
	{
		...
		Vec2 v = new Vec2 (x, y);
		//System.out.println("roam to : " + v.toString() + " with angle :" +angle);
		/*
		*/			
		addVelocity(v);
	}
	...
	public void setAngleFromVelocity(Vec2 v)
	{
		...
		//System.out.println("v_div = " + v_div + ", v:" + v.toString() + "angle_r:" + angle_r);
		//Float angle_d = new Float(Math.atan(v_div));
		
		//Float angle = new Float(angle_d);
		if (!angle_r.isNaN())
		{
	...
	public void addVelocity(Vec2 v)
	{
		...
		//System.out.println("move to  : "+ v.toString() + ", velocity:" + myBody.m_linearVelocity.toString());
		//", body a : " + Math.toDegrees(myBody.getAngle()) +", angle : " + Math.toDegrees(angle_d) 
		//myBody.m_linearVelocity.x = 0;
		//myBody.m_linearVelocity.y = 0;
		//myBody.applyForce(v, myBody.getWorldCenter());
	...
	public Vec2 MoveToWOAObject(WOAObject aim)
	{
		if (aim != null)
		{
			if (aim.getPos()._map.equals(getPos()._map))
			{
				...
				//System.out.println("apply_force = " + apply_force.toString());
	...
	*/
	...
	/*
	public void smellPheromone(Pheromone p)
	{
		
		//System.out.println("smelling pheromone : " + p.toString());
		if (currentMission == null)
		{

			//if (p.mission.target != null)
			{
				currentMission = new Mission(this, p.current_track.target);
				
				
				//currentMission.piste_completed = true;
				//currentMission.firstPheromone = p.mission.firstPheromone;
				//currentMission.homePheromone = p.mission.homePheromone;
				
				//Collection<Track> tracks = p.tracks.values();
				currentMission.targetTrack = p.current_track;
				currentMission.currentPheromone = p;
				currentMission.state = 3;
				//System.out.println("mission has been cloned : " + toString() + ", mission : " + currentMission.toString());				
			}
			// set target
			//currentMission.target = p.mission.target;
			// go to target
			//currentMission.state = 1;
			
			//currentMission = p.mission.clone(this);
		}
	...
	*/
	public void smellPheromone(NodePheromone p)
	{
		...
		{		
			...
			{				
				...
				{
					//System.out.println("nice piste but don't need more ants : " + p.id + ", motivation[" + p.tn.getMotivation() + "]");
					if (p.tn.getMotivation() > 0)
					{
						//System.out.println("here is the situation : " + p.tn.antRequested.toString());
			...
			{
				//System.out.println("no target on phoeromones : " + p.id);
				p.addToGarbage();
			}
		}
		else
		{
			//System.out.println("sorry I[" + id + "] have another mission : " + currentTrackMission);
		}
	}
	...
	{		
//		
//		if (currentBox.m_boundaryListener.getContact())
//		{
//			currentBox.m_boundaryListener.noContact();
//			//currentBox.m_world.destroyBody(this.myBody);
//			//System.out.println("ant is out");
//			boundaryViolated();			
//			//bodyDef = new b2BodyDef();
//			//create_circle();
//		}
//		
		//Vec2 old_vel = myBody.m_linearVelocity;
		//myBody.applyForce(new Vec2(1, 0), myBody.getPosition());
		//System.out.println("" + myBody.m_linearVelocity.x);
		//addVelocity(new Vec2(10, 0));		
		...
		if ((lifestep % 15) == 0)
		{
			...
			if (pheromones != null)
			{
				...
				{
					//System.out.println("See " + pheromones.size() + " pheromones & my mission : " + currentTrackMission);
				}
				for (WOAObject wo : pheromones)
				{
					//System.out.println("smell :" + wo.id);
		...
		{				
			...
			if ((lifestep % 10) == 0)
			{
				...
				if (target != null)
				{					
					//MoveToWOAObject(target);
					//currentMission = new Mission(this, target);
					//if (target.tracknodes.get(getColony().getQueen().player) != null)
					{
					//	for (TrackNode tn : tracknodes.getColony().getQueen().player)
						{
