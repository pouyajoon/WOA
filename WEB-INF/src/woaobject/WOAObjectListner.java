...
public class WOAObjectListner {
		...
		if (appli != null) {
			if (appli.appScope != null) {
				ISharedObjectService service = (ISharedObjectService) getScopeService(
						appli.appScope, ISharedObjectService.class,
						SharedObjectService.class, false);
				service.createSharedObject(appli.appScope,
						"woalshare_" + ab.id, false);
				so = service.getSharedObject(appli.appScope, "woalshare_"
						+ ab.id, false);
	...
	public void remove_o(Integer id) {
		try {
			...
			if (r != null) {
				ArrayList<WOAObject> objscreen = all_objects_screen
						.get(r.pos._map);
				objscreen.remove(r);
				all_objects.remove(id);
			} else
				System.out
						.println("trying to remove a no present WoaObject inside all_objects");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
		}
