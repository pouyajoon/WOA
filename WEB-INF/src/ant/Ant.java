...
public class Ant extends WOAObject {
	...
	public Ant(Colony c) {
		...
		// adn.set("acceleration", new Property("acceleration", (1 -
		// (adn.getMax("size") * 10) / 100) * 400 + adn.getMax("speed_max")));
		// how many food the ant can take by step
		adn.set("food_collect_capacity", new Property("food_collect_capacity",
				2.0f));
		...
		// setMyBody(c.world.antBox.create_body(adn.getCurrent("size"),
		// adn.getCurrent("weight"), c.myBody.getPosition()));
	...
	public void createBody(AntBox _targetBox) {
		Body b = _targetBox.CreateJCircleBody(getPos().getVec2(),
				adn.getCurrent("size"), adn.getCurrent("weight"), 0.0f);
		setMyBody(b);
	...
	public void boundaryViolated() {
		...
		AntBox boxTarget = currentBox.world.antzone.getTargetBoxUsingDirection(
				currentBox, direction);
		pos = currentBox.world.antzone.getTargetPosition(currentBox, boxTarget,
				direction, myBody.getPosition());
		destroyBody();
		createBody(boxTarget);
	}

	/**
	 * calculate how many the ant can collect and the food capacity and collect
	 * it
	 * 
	 * @param other
	 *            : the object to eat
	...
	public void eatFood(WOAObject other) {
		...
		float ant_food_place = ant.adn.getMax("storage_food")
				- ant.adn.getCurrent("storage_food");
		...
		if (ant_food_place - ant.adn.getCurrent("food_collect_capacity") > 0.0f) {
			...
			ant_food_recolt_capacity = ant.adn
					.getCurrent("food_collect_capacity");
		} else {
		...
		if ((other.adn.getCurrent("storage_food") - ant_food_recolt_capacity) > 0) {
			// if the ant can collect more stock than food capacity, set the ant
			// collect as max
	...
	public AABB viewZone() {
		...
		// Make a small box.
		Vec2 d = new Vec2(adn.getCurrent("view_distance"),
				adn.getCurrent("view_distance"));
	...
	public ArrayList<WOAObject> lookFor(String[] search_types) {
		...
		for (int j = 0; j < shapes.length; j++) {
			...
			if (shapeBody.equals(this.myBody))
				continue;
			if (shapeBody.isStatic() == false) {
				if (shapes[j].m_body.getUserData() != null) {
					WOAObject woaobject = (WOAObject) shapes[j].m_body
							.getUserData();
	...
	public void releaseMandible() {
		if (mandible != null) {
			WOAObject woaObjectOther = (WOAObject) mandible.m_body2
					.getUserData();
	...
	public Vec2 MoveToWOAObject(WOAObject aim) {
		if (aim != null) {
			...
			{
				Integer direction = AntZone.NextBoxInMap(currentBox,
						aim.currentBox);
				System.out
						.println("need to change map, direction " + direction);
	...
	public void smellPheromone(NodePheromone p) {
		if (currentTrackMission == null) {
			if (p.tn.toTargets.size() > 0) {
				...
				if (stillRequestAnt) {
					currentTrackMission = new MissionTrack(currentBox.world,
							target);
	...
	public void life() {
		if ((lifestep % 15) == 0) {
			ArrayList<WOAObject> pheromones = BodyUtils.lookFor(this,
					"pheromone", viewZone());
			if (pheromones != null) {
