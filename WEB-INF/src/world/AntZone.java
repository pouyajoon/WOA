...
public class AntZone {
	...
	public static Integer NextBoxInMap(AntBox boxSource, AntBox boxTarget) {
		...
		System.out.println(boxSource.id + ", " + boxTarget.id + " | source:["
				+ source_row + ", " + source_column + "], target:["
	 ...
	 * 
	 * @param currentBox
	 *            : last box of the woa object
	 * @param targetBox
	 *            : new box of the woa object
	 * @param direction
	 *            : direction of the box swith
	 * @param body_position
	 *            : current object body position
	 * @return
	 */
	public static Position getTargetPosition(AntBox currentBox,
			AntBox targetBox, int direction, Vec2 body_position) {
		Position targetPos = new Position(targetBox.id, body_position.x,
				body_position.y);
		switch (direction) {
