...
//    	registerExample(new Domino(this));
	...
	public void postStep()
	{
		try
		{
			...
			Iterator<WOAObject> vb = antworld.hasViolateBoundary.values().iterator();
			while (vb.hasNext())
			{
			...
			while (i.hasNext())
			{
				...
				if (appli != null && appli.appScope != null)
				{
		    		...
		    		ServiceUtils.invokeOnAllConnections(appli.appScope, "diedOnServer", new Object[]{w.id});					
				}
	...
	public void preStep() 
	{
		try
		{
			...
			Iterator<WOAObject> woaoi = antworld.woal.all_objects.values().iterator();
			while (woaoi.hasNext())
			{
...
//			}	
//			// life of pheromones
			//System.out.println("node pheromones size : " + antworld.nodepheromones.size());
		...
		{
			antworld.log.debug("concurrent modification exception catch on creating a new object while iterating the big list");
		}
		catch (Exception e)
		{
			e.printStackTrace();
...
//    					float timeStep = test.settings.hz > 0.0f ? 1.0f / test.settings.hz : 0.0f;
//    					test.m_world.step(timeStep, test.settings.iterationCount);
    				}    		
    ...
    public void keyPressed() {
    	...
    	//System.out.println(keyCode + ", " + PApplet.TOP);
    	if (keyCode == 38) // TOP
    	{
    		
    		int testIndex = currentTestIndex - antworld.antzone.zone_size;
    		if (testIndex < 0)
    		{
    			currentTestIndex = tests.size() + testIndex;
    		}
    		else
    		{
    	...
    	{
    		int testIndexBottom = currentTestIndex + antworld.antzone.zone_size;
    		if (testIndexBottom > tests.size() - 1)
    		{
    			currentTestIndex = testIndexBottom - tests.size();
    		}
    		else
    		{
    			currentTestIndex = testIndexBottom;
    		}
      		//System.out.println(currentTestIndex);
    		currentTest = tests.get(currentTestIndex);
    		currentTest.needsReset = false;
    	...
    	if (key == 'f') 
    	{
    		...
    		AntBox currentBox = (AntBox)currentTest;
    		Food food2 = new Food(currentBox, new Position(currentBox.id, currentTest.mouseWorld.x, currentTest.mouseWorld.y), quantity);	
    		//System.out.println("Mouse cursor : " + currentTest.mouseWorld.toString() + "||" + pmouseX + "," + pmouseY);
    		getAntWorld().foods.add(food2);

//    		ServiceUtils serverutils = new ServiceUtils();
//			serverutils.invokeOnAllConnections(appli.appScope, "helloserver", new Object[0]);
    	}
    	...
    	if (key == 'q') 
    	{
			...
			//players.put("pouya", pouya);
	   		//float x = (float)(Math.random() * 500f) + 50;
	   		//float y = (float)(Math.random() * 500f) + 50;
    		AntBox currentBox = (AntBox)currentTest;
			new Queen(currentBox, new Position(currentBox.id, currentTest.mouseWorld.x, currentTest.mouseWorld.y), pouya);
    	}
    	
    	
    	if (key == 'h')
    	{
    		//Pheromone p = new Pheromone(getAntWorld(), new Position(0, 450, 250));
//    		GraphTrack gt = new GraphTrack(getAntWorld());
    		((AntBox)currentTest).CreateJCircleBodyStatic(new Vec2(currentTest.mouseWorld.x, currentTest.mouseWorld.y), 20, 0, 0.0f);
//    		NodePheromone p = new NodePheromone((AntBox)currentTest, new Position(0, 450, 250), gt, new TrackNode(null, gt));
    		//getAntWorld().pheromones.add(p);
    	}
