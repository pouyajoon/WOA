...
public class ProcessingDebugDraw extends DebugDraw {
	...
	public ProcessingDebugDraw(PApplet pApplet) {
		...
		screen = this;
		g = pApplet;
		m_font = g.createFont("Calibri",12);//-Bold-14.vlw");
		fontHeight = 14.0f;
	}
    ...
    public void setCamera(float x, float y, float scale) {
    	...
    	float transX = PApplet.map(x,0.0f,-1.0f,g.width*.5f,g.width*.5f+scale);
    	float transY = PApplet.map(y,0.0f,yFlip*1.0f,g.height*.5f,g.height*.5f+scale);
    	float scaleFactor = scale;
    	viewportTransform.setCamera(transX, transY, scaleFactor);
    	//System.out.println("set camera to : " + transX + ", " + transY + ", " + scaleFactor);
    }
    
	@Override
	public Vec2 getWorldToScreen(Vec2 world) {
	...
	public Vec2 getWorldToScreen(float x, float y) {
		return getWorldToScreen(new Vec2(x,y));
	}
	
	@Override
	public Vec2 getScreenToWorld(Vec2 screen) {
	...
	public Vec2 getScreenToWorld(float x, float y) {
		return getScreenToWorld(new Vec2(x,y));
	}
	...
	public void drawPolygon(Vec2[] vertices, int vertexCount, Color3f color) {
		...
		for (int i = 0; i < vertexCount; ++i) {
			...
			Vec2 v1 = getWorldToScreen(vertices[i]);
			Vec2 v2 = getWorldToScreen(vertices[ind]);
	...
	public void drawSolidPolygon(Vec2[] vertices, int vertexCount, Color3f color) {
		...
		g.beginShape(PApplet.POLYGON);
		for (int i = 0; i < vertexCount; ++i) {
			Vec2 v = getWorldToScreen(vertices[i]);
			g.vertex(v.x, v.y);
		}
		g.endShape();
	...
	public void drawPoint(Vec2 position, float f, Color3f color) {
		position = getWorldToScreen(position);
...
//		position = getWorldToScreen(position);
//		localOffset = getworldToScreenVec(localOffset);
