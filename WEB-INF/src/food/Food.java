...
public class Food extends WOAObject 
{
	public Food(AntBox w, Position pos, float size) {
		...
		//adn.getCurrent("size");
		Body b = currentBox.CreateJCircleBody(pos.getVec2(), getSize(), getWeight(), 0.0f);		
		//b.setUserData(this);
		setMyBody(b);
	...
	public void life()
	{
		...
		if (myBody != null)
		{
			...
			{
				//System.out.println("need resize to : " + adn.getCurrent("storage_food") + ", current radius : " + c.getRadius());
				resize();
			}			
	...
	public void localfinalize()
	{
		...
		if (tracknodes.size() > 0)
		{
			...
			{
				//if (tn != null && tn.toHome != null && tn.toHome.toTargets != null)
				{
					//TrackNode tnHome = (TrackNode)tn.toHome.toTargets.get(this);	
				}
	...
	public void resize()
	{
		...
		newCircleDef.density = getWeight();
		newCircleDef.friction = f;
		newCircleDef.restitution = rest;
		
		myBody.createShape(newCircleDef);			
	...
	public void collide(WOAObject other)
	{
		...
		{
			//System.out.println("stop eating me : " + adn.getCurrent("storage_food") + "@body :" + myBody.getPosition().toString());
		}
	}
